...
module.exports = {
 ...
 execute: async(interaction) => {
  ...
  if(command.cooldown) {
    ...
    if(cooldown.has(`${command.name}${interaction.user.id}`)) return interaction.reply({ content: `Cooldown şuan aktif lütfen <t:${Math.floor(waitedDate / 1000)}:R> tekrar deneyin.`, ephemeral: true}).then(msg => setTimeout(() => interaction.deleteReply(), cooldown.get(`${command.name}${interaction.user.id}`) - Date.now()))
